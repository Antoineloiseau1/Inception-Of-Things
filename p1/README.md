...
- **ServerWorker**: Kubernetes worker node. This machine is responsible for actually running our applications inside containers. When the control plane decides to start or stop containers, it sends instructions to the ServerWorker. We'll install K3s in *agent mode*: this machine joins the cluster and waits for the Server to tell it what workloads to run.

- **Vagrant** creates the VMs that will be the server and the agent of the K3s cluster, relying here on virtualBox.
